...
case ${ID}-${VERSION_ID} in
      ...
      *)
        ...
        yum -y -q update
        yum install -y -q sudo hostname gzip wget vim java-1.8.0-openjdk-devel openssl zlib compat-libstdc++-33 snappy openssh-clients openssh-server initscripts nc unzip 
        NOARCH="noarch"
...
if [ ! -d $pkg_dir  ] ; then
	...
	wget -qO spark.zip $BIGTOP_BASE_URL=$SPARK,OS=$BIGTOP_OS_TYPE$arch/lastSuccessfulBuild/artifact/*zip*/archive.zip
        ...
        unzip hadoop.zip; unzip spark.zip; find -name "*.rpm" -o -name "*.deb" | xargs -0 -I {} mv {} .; rm -rf hadoop; rm -rf spark; rm -f *.zip
fi
